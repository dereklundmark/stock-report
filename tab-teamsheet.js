...
function renderTeamSheet() {
  ...
  const tierNote = tierDiff <= 5
    ...
    : `${dTopTier > eTopTier ? 'P1' : 'P2'} leans more heavily into 2026 Top 15 characters (${dTopTier}% vs ${eTopTier}%).`;

  const dG = genderSplit(dSlice), eG = genderSplit(eSlice);
  const genderNote = (dG.M > 60 && eG.M > 60)
    ? `Both lean heavily male. ${eG.M < dG.M ? 'P2' : 'P1'} mixes it up slightly more.`
  ...
  const sdP1 = sd.filter(s => s.won === 'D');
  const sdP2 = sd.filter(s => s.won === 'E');
  ...
  <div style="background:#0F1217;border-radius:14px;padding:24px;margin-top:14px;border:1px solid rgba(255,255,255,.05);position:relative;overflow:hidden;">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:16px;position:relative;">
      <div style="display:flex;flex-direction:column;gap:8px;">
        ...
        ${sdP1.map((s, i) => `<div style="background:rgba(255,82,70,.04);border:1px solid rgba(255,82,70,.1);border-radius:10px;padding:14px;display:flex;align-items:center;gap:10px;">
      ...
      <div style="display:flex;flex-direction:column;gap:8px;">
        ...
        ${sdP2.map((s, i) => `<div style="background:rgba(31,160,224,.04);border:1px solid rgba(31,160,224,.1);border-radius:10px;padding:14px;display:flex;align-items:center;gap:10px;">
