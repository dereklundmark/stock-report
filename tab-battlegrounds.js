...
function renderBattlegrounds() {
  ...
  const venues = [
    { label:'IN-PERSON', n:v.person.n, d:v.person.d, e:v.person.e, dPct:ipD, ePct:100-ipD, note:"On the couch, P2's edge grows." },
  ...
  const platCards = [
    { label:'PLATFORMS ON',  n:pl.on.n,  d:pl.on.d,  e:pl.on.e,  dPct:pl.on.dPct,  ePct:100-pl.on.dPct,  note:'P1 closes the gap with platforms.' },
    { label:'PLATFORMS OFF', n:pl.off.n, d:pl.off.d, e:pl.off.e, dPct:pl.off.dPct, ePct:100-pl.off.dPct, note:"Flat stages suit P2's playstyle." }
  ];
  ...
  if (dow.length) {
    ...
    dowNote = `${best.day} is P1's best day (${best.dPct}%). ${worst.day} is his worst (${worst.dPct}%). ${worst.day} is P2's stronghold (${100 - worst.dPct}%).`;
  }
...
<div style="max-width:1080px;margin:0 auto;padding:34px 18px 90px;">
  ...
  <div style="background:#0F1217;border-radius:14px;padding:24px;border:1px solid rgba(255,255,255,.05);margin-bottom:14px;">
    ...
    <div style="overflow-x:auto;-webkit-overflow-scrolling:touch;margin:0 -4px;padding:0 4px;">
      <div style="display:grid;grid-template-columns:repeat(${monthly.length},minmax(48px,1fr));gap:8px;align-items:end;height:200px;min-width:480px;">
